# usage: visualize the time/batch effect correction for viral_high_group
# date updated: 02/19/2020
# author: Zhiyuan Yao
# other scripts needed : Complete_tables_generation_high.R
...
Completelist_high <- read.table('/Users/yzhiyuan/workspace/VEEV/VEEV_Rproject/table/Completelist_high.tsv', 
                               sep='\t', row.names=1, header=T) 

# mark genes not affected by time/batch effect 
a <- c('spearman_cor', 'pearson_cor_infect', 'pearson_cor_uninfect')
b <- Completelist_high[,a]
...
ggplot(b, aes(pearson_cor_uninfect, pearson_cor_infect)) + 
